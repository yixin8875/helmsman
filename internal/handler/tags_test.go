...
func newTagsHandler() *gotest.Handler {
	testData := &model.Tags{}
	testData.ID = 1
	...
	d.IDao = dao.NewTagsDao(d.DB, c.ICache.(cache.TagsCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &tagsHandler{iDao: d.IDao.(dao.TagsDao)}
	iHandler := h.IHandler.(TagsHandler)
...
func Test_tagsHandler_Create(t *testing.T) {
	h := newTagsHandler()
	defer h.Close()
	testData := &types.CreateTagsRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Tags))
...
func Test_tagsHandler_DeleteByID(t *testing.T) {
	h := newTagsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Tags)
...
func Test_tagsHandler_UpdateByID(t *testing.T) {
	h := newTagsHandler()
	defer h.Close()
	testData := &types.UpdateTagsByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Tags))
...
func Test_tagsHandler_GetByID(t *testing.T) {
	h := newTagsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Tags)
...
func Test_tagsHandler_List(t *testing.T) {
	h := newTagsHandler()
	defer h.Close()
	testData := h.TestData.(*model.Tags)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &types.ListTagssRequest{query.Params{
		Page:  0,
		Limit: 10,
